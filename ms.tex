...
We report the discovery of a young ($\tau \sim \clage$), low-mass ($M \sim 1200~\msun$), metal-poor ($\feh \sim \clfeh$) stellar association in the Milky Way halo: \clustername.
From isochrone fitting to follow-up \decam\ imaging of the cluster, we also measure its distance, $D \approx \cldist$.
...
At this location relative to the LMC, the measured distance is inconsistent with predictions from models of the LMC/SMC interaction and infall into the Milky Way that do not account for ram pressure and gas interaction with Milky Way disk.
The estimated age of \clustername\ is consistent with the time of last passage of the leading arm gas through the Galactic midplane, and we therefore speculate that this star-formation event was triggered by the last disk midplane passage, which occurred at a Galactocentric radius $R \approx 30~\kpc$.
However, most details of this idea remain a mystery: the Magellanic stream has low column density, the Milky Way disk at this large radius has low gas density, and the relative velocity of the leading arm gas and Milky Way gas is large. However it formed, the discovery of a young stellar cluster in the Milky Way halo presents an interesting opportunity for study.
\end{abstract}
...
More recent hydrodynamical simulations of the MCs showed that the repeated encounters even after the infall between the MCs strip gas not only from the SMC, but also from the LMC, and this tidally stripped gas can create filamentary gas structures both in the leading and trailing MS \citep{Pardy:2018}. Many observational studies \citep[e.g.,][]{Olsen:2011, Noel:2013, Mackey:2016, Carrera:2017, Choi:2018a, Choi:2018b, Zivick:2018, Belokurov:2019} have been made in the Magellanic system that are available to provide strong constraints on modeling the interaction history of the \lmcsmc\ in detail.  
...
The clear over-density in the cluster field (blue) are stars in \clustername. \yumi{can you add the mean proper motion vector of the cluster on this figure or figure 1?}}
...
The control fields are designed to, together, have the same total area as the cluster field, and were chosen to have similar latitudes as the cluster field in the Magellanic stream coordinate system \citep{Nidever:2008}. \yumi{by assuming a smooth background stellar density at similar latitudes?}
...
We run the sampler with 32 walkers for 256 steps as burn-in, then reset the sampler and run for an additional 512 steps. \yumi{why this number of walkers and steps? depends on number of fitting parameters and autocorrelation time?}
We then thin \yumi{thin? out of my own curiosity: why thinning is needed?} the resulting chains by taking every 16th sample, leaving a total of 1024 samples; We use these samples to estimate the median posterior parameter values and uncertainties,
...
The prior and bounds on $A_V$ are set to prefer small and reasonable values of extinction for this moderately high Galactic latitude region. \yumi{Av is known to follow a log-normal distribution.}
...
We are developing tools to perform star cluster parameter inference in this way (Morton et al., in prep.), but here we adopt a simple hack to allow us to instead perform individual posterior samplings and then combine those samplings into a hierarchical inference. \yumi{I like this fancy way. But it would be helpful for readers (potential users of this method) if you can briefly describe the strength of this methodology over the standard isochrone fitting for a star cluster?}

The brightest stars in the cluster have very precise \decam\ photometry, and have precise astrometry from \gaia.
Given their location in the CMD, these stars must be young, and so also have much better constrained stellar parameters. \yumi{I am not convinced by that bright stars have better constrained stellar parameters. Their color and magnitude are sensitive to dust and effective temperature, but their age and metallicity would not constrained better than fainter stars. I suggest to remove the second half of the previous sentence.}
...
Over-plotted in \figurename~\ref{fig:hierarch-iso} (blue, solid line) is a \acronym{MIST} isochrone with the median posterior parameters derived from the hierarchical modeling, shifted to the median distance and extincted given the median $A_V$ value. \yumi{How our Av is different from Schlegel map (https://irsa.ipac.caltech.edu/cgi-bin/bgTools/nph-bgExec)? Their Av is about 0.2. Are we consistent with them within uncertainty?}
...
\begin{center}
    \begin{tabular}{ r | l | l}
        ...
        \toprule
    \end{tabular}
\caption{This table summarizes the measured or inferred kinematic and stellar population parameters of the young halo stellar cluster \clustername. \yumi{please include uncertainties.}
...
The MS gas is most visible and densest at the highest velocities where the LA II feature is found, around $L_{\rm MS} \lesssim 61^\circ$ and $B_{\rm MS} \sim -12^\circ$, but the lower panels (i.e. slower gas) still displays some of the same morphology as the highest velocity slice. \yumi{I was confused when I read this for the first time. You might want to mention earlier that HI velocities in the LA is measured to be larger than 150 kmc/s. Otherwise, the reader who are not familiar with the LA would misinterpret that the highest gas density around \clustername\ is found in the bottom panel.}
...
Previous studies have detected young stars in the LA and in the periphery of the LMC \citep{Casetti-Dinescu:2014, Moni-Bidin:2017}, however, this is the first time that an entire young star cluster has been detected so far.% from the Clouds.
...
It also provides an opportunity to study star formation in a unique environment (i.e., low gas density and low metallicity), unlike that of the Milky Way disk or any other cluster-forming region.
