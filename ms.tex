...
\figurename~\ref{fig:cmds} shows the \gaia\ color-magnitude diagram (left) and the \acronym{GALEX} NUV minus \acronym{PS1} $g$-band vs. \gaia\ \bprp\ color-color diagram (right) from the cross-matched photometry.
...
The red line in right panel of \figurename~\ref{fig:cmds} is an isochrone (from MIST; \citealt{TODO}) for a $50~\textrm{Myr}$, $\feh = -1$ stellar population.

% Notebook: Figure-CMDs
\begin{figure}
\centering
\includegraphics[width=\textwidth]{figures/gaia-galex-cmd.pdf}
\caption{Color-magnitude and color-color diagrams with \gaia\ ($G$, BP, RP), \acronym{GALEX} (NUV), and \acronym{PS1} ($g$) magnitudes.
...
Green polygon in left panel highlights the over-density of blue, comoving stars identified by the search described in \sectionname~\ref{sec:data}.
Red line in right panel shows the zero-age main sequence of a $50~\textrm{Myr}$, $\feh = -1$ stellar population from the MIST isochrone library \citep{TODO}.
}
\label{fig:cmds}
\end{figure}
