...
\title{Star formation in the leading arm of the Magellanic stream triggered by passage through the Galactic midplane}
...
While Sagittarius was likely stripped of its neutral gas long ago \citep{Burton:1999, Tepper-Garcia:2018}, the \lmcsmc\ system contains $\approx 5\times 10^8~\msun$ of HI gas \citep{Bruns:2005}, which extends into leading and trailing gas streams that wrap nearly $\approx 200^\circ$ around the sky \citep{Putman:1998, Bruns:2005, Nidever:2010}.
Streams formed from tidally stripped material --- such as the Sagittarius or Palomar 5 \citep{Odenkirchen:2001} stellar streams, or gas streams such as the Magellanic stream (MS) --- encode information about the orbital history and future trajectory of their progenitor system \citep[e.g.,][]{Johnston:1999}.
...
(Summarize magellanic stream stuffs)
...
However, later measurements of the proper motion of the LMC (\citealt{Kallivayalil:2006}, later confirmed by \gaia\ \citealt{van-der-Marel:2016}) and improved models for the infall dynamics \citep{Besla:2007} suggest that the \lmcsmc\ system could be on its first passage through the Galaxy \citep{Besla:2010, Besla:2012}.
In addition, the orbit of the LMC implied by the new proper motion measurement appears to be offset by $\approx 10^\circ$ from the MS \citep{Besla:2010}.
...
(More recently, proper motions (confirmed by Gaia) show that orbit of LMC differs from path of MS)
(Difficulty reproducing offset by invoking tidal stripping + ram pressure, interaction history important)
(But, questions remain because leading stream)
...
The leading arm, however, ...
A small number of young OB stars have been found in the vicinity of the leading arm gas, with radial velocities consistent with having formed from the leading MS \citep{Casetti-Dinescu:2014, Zhang:2017}.
...
(Young OB stars tentatively associated with leading (Casetti-D))

However, the connection between the Magellanic clouds and the Magellanic Stream (MS)


For example, by modeling the \todo{XX} \citep{Besla:2010}, it is now understood that the \lmcsmc\ is likely on its first approach through the Galaxy, which has major implications for understanding the properties of the system itself and its effect on Galactic structure such as the bar, spiral arms, and outer disk \citep[e.g.,][]{Gomez:xxxx, Laporte:20xx}.


(But lacks distance constraints)
(Many people search for stars associated with the stream, but must be diffuse)

Gas associated with the LMC--SMC system has continued to form stars:

(Gas near the LMC/SMC is forming stars \citep{Casetti-Dinescu:2014, MoniBidin:2017}, and cite Deason/Belokurov bridge paper)


For the Magellanic Clouds, observations of tidally stripped gas or stars should help constrain the LMC--SMC interaction history and infall, and will help inform the expected response of the Milky Way to this massive accretion event \citep[e.g.,][]{Gomez:xxxx, Laporte:20xx}.



Leading arm features - ??? Disrupted satellites of the LMC?? Fox 2015 paper
