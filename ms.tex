...
We use a robust linear fit to cross-matched sources (that appear in both PS1 and our observations) to obtain a zero-point and color-term (no color-term was used for the u-band) to derive color transformations relating our instrumental DECam photometry to calibrated PS1 grizy and SkyMapper u-band photometry (see \tablename~\ref{tbl:transphot} for the derived transformation coefficients).
...
The larger scatter in u-band---larger than the uncertainty in the individual photometry---is due to the significant differences between the DECam and SkyMapper u-band passbands as well as the u-band's sensitivity to both temperature and metallicity. 
}
