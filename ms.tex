...
% - You refer to the prior on EEP as "uniform."  While on the surface this is what it looks like in 'isochrones', that is not quite correct (and would be erroneous).  Rather, the effective prior on EEP p(E) is transformed from a Salpeter-IMF mass prior, where p(E) = p(M) * |dM/dEEP|.  Not sure the most succinct way to communicate this, but Table 1 is incorrect as is.
% - I think Figure 6 would benefit from showing the non-cluster members as well, maybe with a low alpha, to demonstrate how the model picks out the sequence like a charm.
...
\begin{center}
    \begin{tabular}{ r | l | l}
        ...
        \toprule
    \end{tabular}
\caption{Inferred properties of \clustername.
...
The colorbar shows the column density of \hi in units of \todo{WHAT UNITS?}.
...
The entire region shown in \figurename~\ref{fig:gass_maps} is on the other side of the Galactic disk with respect to the Magellanic clouds, meaning that all of the gas in this region has passed through the Galactic midplane.
The similar filamentary structure in the high and lower velocity gas therefore raises the question: Did the low velocity filaments originate in the Magellanic stream, but were decelerated through interactions with Milky Way gas? \todo{Is this an accepted explanation? I don't see much discussion of these features in other papers}
...
We can therefore use the line-of-sight velocity of the gas as a proxy for the line-of-sight velocity of the stars in \clustername\ to study the Galactic orbit of the cluster, as discussed in the next section.
