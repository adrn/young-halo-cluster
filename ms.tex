...
% - release data for the candidate members!
...
\title{\textbf{Discovery of a disrupting open cluster far into the Milky Way halo:\\
...
However, most details of this idea remain a mystery: the Magellanic stream is low column density, the Milky Way disk at this large radius has low gas surface density, and the relative velocity of the leading arm gas and Milky Way gas is large. However it formed, the discovery of a young stellar cluster in the Milky Way halo presents an interesting opportunity for study.
\end{abstract}
...
%Yumi: Intro can be condensed and go into the point quickly.
...
% Yumi: this paragraph needs to be clear. The MS is the trailing gas and the LA is the leading gas. Both together is 200 degrees long.
...
Shaded pixels (orange) show regions of high density and points (black markers) are plotted when there are fewer than 8 stars per pixel. \yumi{It seems you made extinction correction on this CMD. If so, please describe it in the text.}
...
The left panel shows the \gaia\ color-magnitude diagram, with the blue over-density highlighted by the polygon (blue) and under-plotted with a $100~\textrm{Myr}$, $\feh = -1.1$ \acronym{MIST} isochrone \citep[red line;][]{Dotter:2016, Choi:2016, Paxton:2011, Paxton:2013, Paxton:2015}.
...
The control fields are designed to, together, have the same total area as the cluster field. \yumi{Any changes if you pick different control fields?}
