...
We use an ensemble Markov chain Monte Carlo (MCMC) sampler \citep[\texttt{emcee};][]{Goodman:XX, Foreman-Mackey:2013} to generate posterior samples over the parameters $(f, \bs{m}, s)$ using the likelihood defined above (\equationname~\ref{eq:likelihood}), and assuming the following prior probability distributions: uniform over the domain $(-5, 5)~\masyr$ for each component of $\bs{m}$, uniform in $f$, and uniform in log-$s$ over the domain $-6 < \ln s < 4$ (with $s$ in units of \masyr).
...
For the cluster, we assume the values are drawn from delta functions in age, metallicity, distance, and extinction, with the centroids of the delta functions $\bs{\alpha} = (\mu_\tau, \mu_{\feh}, \mu_D, \mu_{A_V})$ as hyperparameters of the hierarchical inference.
...
\begin{center}
    \begin{tabular}{ c | c | c }
        ...
        Parameter & Prior & Bounds \\
        \toprule
        $\mu_\tau$ & uniform & $(1~\textrm{Myr}, 1~\textrm{Gyr})$ \\
        $\mu_{\feh}$ & uniform & $(-2, 0)$ \\
        $\mu_{A_V}$ & uniform & $(0, 1)~\textrm{mag}$ \\
        $\mu_D$ & uniform & $(1, 100)~\kpc$ \\
        $f$ & uniform & (0, 1) \\
        \toprule
    \end{tabular}
...
Over-plotted (blue, solid lines) are \acronym{MIST} isochrones computed from 128 randomly chosen posterior samples, shifted to the distance of the given sample and extincted given the $A_V$ value of the sample.
