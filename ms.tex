...
We initially select all stars from \gaia\ with $\textrm{parallax} < 1$, $-0.5 < \bprp < 0$, $G < 20$, and Galactic latitude $|b| > 20^\circ$ (see \sectionname~\ref{sec:queries} for database query).
...
We then query all objects from the \gaia\ \DR{2} catalog within a polygon centered on the nominal position of the group (see above), with a width of $12^\circ$ and a height of $8^\circ$ in the Magellanic stream coordinate system \citep{Nidever:2008} (see \sectionname~\ref{sec:queries} for the database query).
